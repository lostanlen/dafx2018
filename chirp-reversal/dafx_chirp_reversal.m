...
% Synthesize chirps.
x = cat(1, repmat(dafx_synthesize_chirps(N/8), 7, 1), zeros(N/8, 1));
...
for j2_tm = 1:nTemporal_scales
    ...
    for j_fr = 1:nFrequency_scales
        ...
        sigmoid = 1 ./ (1 + exp(- 5.0 * linspace(-3.5, 4.5, n_times).'));
        ...
        gauss_ft = exp(-omega.*omega / (2*j2_sigma^2));
...
%% Tukey window.
tukey_length = (0.75 * N);
t = linspace(0,1,tukey_length)';
per = 0.1; 
tl = floor(per*(tukey_length-1))+1;
th = tukey_length - tl + 1;
w = [ ...
    ((1+cos(pi/per*(t(1:tl) - per)))/2); ...
    ones(th-tl-1,1); ((1+cos(pi/per*(t(th:end) - 1 + per)))/2) ...
];
padding = zeros((N-tukey_length)/2, 1);
tukey_highpass = [padding; w; padding(1:end)];
plot(tukey_highpass);
...
while (iteration <= rec_opts.nIterations) && ...
    ...
    % Griffin-Lim.
     if mod(iteration, 25) == 0 && iteration <= 200
         iterations{1+iteration} = y;
         y_ft = fft(y);
         y_ft((1+(end/2)):end) = 0;
         y_hilbert = 2 * ifft(y_ft);
         y_amplitude = abs(y_hilbert);
         y = (x_amplitude ./ (1e-6 +y_amplitude)) .* y;
         y_ft = fft(y);
         y_ft = y_ft .* tukey_highpass;
         y = real(ifft(y_ft));
